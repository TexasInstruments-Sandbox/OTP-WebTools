#!/usr/bin/env python3
"""
Explore the Use Case sheet in detail to find the actual use case definitions
"""
...
def explore_use_case_sheet(zip_ref, shared_strings):
    ...
    # Parse sheet - get MORE rows to find use cases
    with zip_ref.open(sheet_file) as f:
        ...
        for row_elem in sheet_data.findall('.//main:row', NS):
            ...
            # Get first 100 rows and first 30 columns
            if row_idx >= 100:
                continue
...
def print_use_case_data(data):
    """Print Use Case sheet data in a readable format"""
    if not data:
        print("  (empty)")
        return
    ...
    max_col = min(max_col, 29)

    # Print header
    print("     ", end="")
    for col in range(min(max_col + 1, 20)):
        col_letter = col_idx_to_letter(col)
        print(f"{col_letter:>15}", end=" ")
    print()

    # Print rows - look for interesting sections
    for row in range(max_row + 1):
        row_data = data.get(row, {})

        # Check if this row has any interesting content (like "Starting Use case" or use case names)
        has_content = False
        for col_val in row_data.values():
...
def main():
    print(f"Exploring Use Case sheet in {EXCEL_FILE}...\n")
